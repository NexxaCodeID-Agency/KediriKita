...
export default function FloatingCards() {
  ...
  useEffect(() => {
    ...
    // Skip Three.js on mobile — too heavy for touch devices
    ...
    const animate = () => {
      ...
      cards.forEach((card, i) => {
        ...
        card.position.x +=
          (POSITIONS[i][0] + mouseX * 0.08 * (i % 2 === 0 ? 1 : -1) -
            card.position.x) * 0.04;
      ...
      // Float vertikal icon
      icon.position.y  = Math.sin(t * 0.55) * 0.055;
      ...
      ringMat.opacity = (scrolled ? 0.55 : 0) + Math.sin(t * 1.2) * 0.08 * (scrolled ? 1 : 0);

      // Mouse parallax ringan pada icon
      icon.position.x  = 1.0 + mouseX * 0.05;
